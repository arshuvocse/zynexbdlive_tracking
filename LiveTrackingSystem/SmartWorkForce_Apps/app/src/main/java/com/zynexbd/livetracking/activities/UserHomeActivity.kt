...
class UserHomeActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val companyName = session.getCompanyName() ?: getString(R.string.app_name)
        binding.textBrandLabel.text = companyName

        val brandLogo = session.getBrandLogo()
        if (!brandLogo.isNullOrBlank()) {
            binding.imageBrandLogo.visibility = android.view.View.VISIBLE
            com.bumptech.glide.Glide.with(this)
                .load(brandLogo)
                .circleCrop()
                .into(binding.imageBrandLogo)
        } else {
            binding.imageBrandLogo.visibility = android.view.View.GONE
        }
